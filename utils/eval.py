...
def plot_trajectory(
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_title(f"Action Dimension {i}: {modality_keys[i]}", fontsize=12, fontweight="bold", pad=10)
    ...
    if save_plot_path:
        # Insert trajectory ID into filename to avoid overwriting
        from pathlib import Path
        save_path = Path(save_plot_path)
        save_path_with_traj = save_path.parent / f"{save_path.stem}_traj_{traj_id}{save_path.suffix}"
        print("saving plot to", save_path_with_traj)
        plt.savefig(save_path_with_traj, dpi=300, bbox_inches="tight")
    else:
        plt.show()

...
class SinglePandaGripperWithFTDataConfig(BimanualPandaGripperDataConfig):
    video_keys = [
        "video.left_view",
        "video.right_view",
        "video.wrist_view",
    ...
    force_normalization_modes = {
        "force.wrist_ft": "min_max",
    }
    action_normalization_modes = {
        "action.end_effector_position": "min_max",
        "action.end_effector_rotation": "min_max",
    ...
    def modality_config(self) -> dict[str, ModalityConfig]:
        """Override to add force modality."""
        config = super().modality_config()
        # Add force modality
        config["force"] = ModalityConfig(
            delta_indices=self.observation_indices,
    ...
    def transform(self):
        """Override to add force transforms."""
        transforms = [
            # video transforms
            VideoToTensor(apply_to=self.video_keys),
            VideoCrop(apply_to=self.video_keys, scale=0.95),
            ...
            # force transforms
            StateActionToTensor(apply_to=self.force_keys),
            StateActionTransform(
                apply_to=self.force_keys,
                normalization_modes=self.force_normalization_modes,
            ),
            # action transforms
            StateActionToTensor(apply_to=self.action_keys),
            StateActionTransform(
            ...
            ConcatTransform(
                video_concat_order=self.video_keys,
                state_concat_order=self.state_keys,
                force_concat_order=self.force_keys,
                action_concat_order=self.action_keys,
            ),
            GR00TTransform(
                state_horizon=len(self.observation_indices),
                ...
                max_state_dim=64,
                max_action_dim=32,
                max_force_dim=6,  # Single 6D F/T sensor
            ),
        ]
        return ComposedModalityTransform(transforms=transforms)
...
class RH20TFrankaWithFTDataConfig(BaseDataConfig):
    """Data config for RH20T dataset with Franka Panda + Force/Torque sensor."""
    video_keys = [
        "video.shoulder_left",
        "video.front_right",
        "video.end_effector",
    ...
    language_keys = []  # No language annotations in RH20T
    observation_indices = [0]
    action_indices = list(range(16))

    state_normalization_modes = {
        "state.tcp_R_11": "min_max",
        "state.tcp_R_21": "min_max",
        "state.tcp_R_31": "min_max",
        "state.tcp_R_12": "min_max",
        "state.tcp_R_22": "min_max",
        "state.tcp_R_32": "min_max",
        "state.tcp_R_13": "min_max",
        "state.tcp_R_23": "min_max",
        "state.tcp_R_33": "min_max",
        "state.tcp_p_x": "min_max",
        "state.tcp_p_y": "min_max",
        "state.tcp_p_z": "min_max",
        "state.gripper_state": "min_max",
    }
    force_normalization_modes = {
        "force.f_R_x": "min_max",
        "force.f_R_y": "min_max",
    ...
    action_normalization_modes = {
        ...
        "action.v_y": "min_max",
        "action.v_z": "min_max",
        "action.gripper_cmd": "min_max",
        "action.joint_1": "min_max",
        "action.joint_2": "min_max",
        "action.joint_3": "min_max",
        "action.joint_4": "min_max",
        "action.joint_5": "min_max",
        "action.joint_6": "min_max",
        "action.joint_7": "min_max",
    }
...
class RH20TFrankaNoFTDataConfig(BaseDataConfig):
    """Data config for RH20T dataset with Franka Panda WITHOUT Force/Torque sensor.

    This config is identical to RH20TFrankaWithFTDataConfig but excludes force data.
    Used for ablation studies to measure F/T sensor contribution.
    """
    video_keys = [
        "video.shoulder_left",
        "video.front_right",
    ...
    # No force_keys - this is the key difference!
    action_keys = [
        "action.w_x",
        "action.w_y",
        "action.w_z",
    ...
    # No force modality - uses base class modality_config()

    def transform(self):
        """Transform WITHOUT force data."""
        transforms = [
            # video transforms
            VideoToTensor(apply_to=self.video_keys),
            VideoCrop(apply_to=self.video_keys, scale=0.95),
            ...
            VideoToNumpy(apply_to=self.video_keys),
            # state transforms (no force transforms!)
            StateActionToTensor(apply_to=self.state_keys),
            StateActionTransform(
                apply_to=self.state_keys,
                normalization_modes=self.state_normalization_modes,
            ...
            # concat transforms (no force_concat_order!)
            ConcatTransform(
                video_concat_order=self.video_keys,
                state_concat_order=self.state_keys,
                action_concat_order=self.action_keys,
...
DATA_CONFIG_MAP = {
    ...
    "single_panda_gripper_with_ft": SinglePandaGripperWithFTDataConfig(),
    "rh20t_franka_with_ft": RH20TFrankaWithFTDataConfig(),
    "rh20t_franka_no_ft": RH20TFrankaNoFTDataConfig(),

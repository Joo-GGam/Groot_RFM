...
class LeRobotModalityMetadata(BaseModel):
    ...
    force: Optional[dict[str, LeRobotStateMetadata]] = Field(
        default=None,
        description="The metadata for the force modality. The keys are the names of each split of the force vector.",
    )
    ...
    def get_key_meta(self, key: str) -> LeRobotModalityField:
        ...
        if modality == "state":
            ...
            ), "Trying to get force metadata for a dataset with no force modality"
            if subkey not in self.force:
                raise ValueError(
                    f"Key: {key}, force key {subkey} not found in metadata, available force keys: {self.force.keys()}"
                )
            return self.force[subkey]
        elif modality == "video":
...
class DatasetStatistics(BaseModel):
    ...
    force: Optional[dict[str, DatasetStatisticalValues]] = Field(default=None, description="Statistics of the force")
...
class DatasetModalities(BaseModel):
    ...
    force: Optional[dict[str, StateActionMetadata]] = Field(default=None, description="Metadata of the force")

...
class GR00TTransform(InvertibleModalityTransform):
    ...
    max_state_dim: int
    max_action_dim: int
    max_force_dim: int = Field(default=0)  # NEW! Force/Torque dimension (0 = disabled)
    state_horizon: int
    action_horizon: int
    ...
    def _prepare_force(self, data: dict):
        ...
        """
        if "force" not in data or self.max_force_dim == 0:
            # No force data or force disabled
            force = np.zeros((self.state_horizon, self.max_force_dim))
            force_mask = np.zeros((self.state_horizon, self.max_force_dim), dtype=bool)
            n_force_tokens = 0  # No force tokens
            return force, force_mask, n_force_tokens

        force = data["force"]
        assert force.shape[0] == self.state_horizon, f"{force.shape=}, {self.state_horizon=}"

        n_force_dims = force.shape[-1]

        # Truncate or pad to max_force_dim
        if n_force_dims > self.max_force_dim:
            force = force[:, : self.max_force_dim]
            n_force_dims = self.max_force_dim
        else:
            # Pad up to max_force_dim if smaller
            force = np.pad(force, ((0, 0), (0, self.max_force_dim - n_force_dims)), "constant")

        # Create mask for real force dims
        force_mask = np.zeros_like(force).astype(bool)
        force_mask[:, :n_force_dims] = True

        # We have 1 force token to represent the entire force/torque
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        # 2.5) Prepare force (NEW!)
        force, force_mask, _ = self._prepare_force(data)
        if self.max_force_dim > 0:  # Only add if force is enabled
